...
def fitness2(pob):
	...
	for x in range(0,tam_cromosomas,3):
		...
		array_repetidos.append(pob[x+1])
	array_repetidos.clear()
	poblacion_fitness.append(suma_fitness)
...
def evolucion_lineal(iterator):	
	...
	mejor_individuis_evulucion.append(mayor)
	#print(mayor,"\n")
	#return mayor
...
def mejor_global_total(count):
	...
	for i in count:
		for j in i:
			if(mejor_global[1]>fitness(j)):
				mejor_global[0]=j
				mejor_global[1]=fitness(j)
...
def main():
	...
	for i in Pobacion:
		#servicio()
		t = threading.Thread(target=fitness2,args=(i,))
		t.start()
		t.join()

	mejor_global_(Pobacion,poblacion_fitness)
	asiganar_fitness_poblacion(poblacion_fitness)
	...
	for i in range(partitions):
		inicio=fin
		fin+=int(tam_poblacion/partitions)
		#print(fin,"======",Pobacion_fitness)
		t = threading.Thread(target=evolucion_lineal,args=(Pobacion_fitness,))
		t.start()
		t.join()

	mejor_indivual=mejor_individuis_evulucion
	#print("\n\n",mejor_indivual,"\n\n")
	mejor_global_total(mejor_indivual)

	tiempo_final = time()
	tiempo_ejecucion = tiempo_final - tiempo_inicial
	print ('TIEMPO DE EJECUCION python :',tiempo_ejecucion)
