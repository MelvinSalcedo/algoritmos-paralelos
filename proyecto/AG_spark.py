...
def evolucion_lineal(iterator,num):	
	...
	if(mayor_==0):
		return mayor[0]
	it = 0
	tam_iter=(num_iteracion/partitions)
	while it<tam_iter:
		padre1=randint(0,mayor_)
		padre2=0
		while (padre1==padre2):
...
def main():
    ...
    count = sc.parallelize(Pobacion,partitions) \
        .map(lambda x: fitness(x)) \
        .collect()

    mejor_global_(Pobacion,count)
    asiganar_fitness_poblacion(count)
    rdd = sc.parallelize(Pobacion_fitness,partitions)
    mejor_individual=rdd.mapPartitions(evolucion).collect()
    mejor_global_total(mejor_individual)
    
    tiempo_final = time()      
    tiempo_ejecucion = tiempo_final - tiempo_inicial
    print("SOLUCION -> " ,count)
    print ('TIEMPO DE EJECUCION SPARK :',tiempo_ejecucion)
